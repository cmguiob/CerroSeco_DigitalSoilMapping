...
knitr::opts_chunk$set(echo = TRUE, fig.align="center", warning = FALSE, echo = FALSE, fig.width = 5.5)
...
theme_update(strip.background = element_blank(),
             ...
             plot.background = element_rect(color =  "#F6F1EB",
                                            fill =  "#F6F1EB", 
                                            size = 2),
             ...
             axis.title = element_text(colour = darken("#c3beb8", 0.2, 
                                                       space = "HCL"), 
                                       face = "bold")) 
...
riet_03  <- riet %>%
  ...
  mutate(horizonte = factor(horizonte, 
                            levels = c("A/E", "Bth", "B", "Bdc2", "B/C2", "Bv2", "B2", "R"))) %>%
  filter(porcentaje != 0)
...
p_riet <- riet_03 %>%
  ...
  theme(axis.text = element_blank(),
        ...
        strip.text.y.left = element_text(colour = darken("#c3beb8", 0.2, 
                                                       space = "HCL"), 
                                        face = "bold",
                                        angle = 0),
...
DRX_CS03 <- DRX_df %>% dplyr::filter(ID == "CS03" & d < 19 & d > 2.295 )
...
p_xrd <- ggplot() +
            geom_line(data = DRX_CS03 %>% dplyr::filter(TRATAMIENTO == "P"), 
                aes(x= d, y= I_plot), 
                size = 0.45,
                color = darken("#f6f2ed", 0.5, space = "HCL")) +
            ...
            geom_line(data = DRX_CS03 %>% dplyr::filter(TRATAMIENTO == "EG"), 
                aes(x= d, y= I_plot), 
                size = 0.1,
                color = darken("#f6f2ed", 0.3, space = "HCL")) +
            geom_line(data = DRX_CS03 %>% dplyr::filter(TRATAMIENTO == "C100"), 
                 aes(x= d, y= I_plot), 
                size = 0.1,
                color = darken("#f6f2ed", 0.2, space = "HCL")) +
            geom_text_repel(data = DRX_CS03,
                  ...
                  nudge_x = 0.27,
                  segment.size = 0.6, #segment thickness
  ...
  #limits in scale_x_continuous would cut the line, not plot area: avoided
            ...
            theme(
                  plot.margin = margin(50, 70, 20, 10), #top, right, bottom, left
                  panel.grid = element_line(
                                  linetype = c("dashed"),
                                  colour = darken("#f6f2ed", 
                                                  0.05, 
                                                  space = "HCL")),
...
p_riet + p_xrd + plot_layout(widths = c(1, 3))
