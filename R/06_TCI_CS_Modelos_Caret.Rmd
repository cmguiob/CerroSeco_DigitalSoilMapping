...
predict_set_knn <- predict(caret_pp, predict_set_knn %>% 
                   ...
                   dplyr::select(-WSB) %>%
                   dplyr::mutate(modelo = factor("k-Nearest Neighbors")) %>%
                   dplyr::mutate(secuencia = factor(NA)) %>%
...
#Crear set para rf
predict_set_rf <- predict_set_knn %>% mutate(modelo = factor("Random Forest"))
...
```{r plot_models, include = TRUE, echo = TRUE}
...
```
```{r save}
ggsave(file = "mapas_modelos.png", plot = p_modelos, device = "png", type ="cairo", path = here::here("graficas"), dpi = 300, width = 5.8, height = 3.8)
